...
const Home = () => {
  ...
  //Keep tracks of checkboxes and corresponding fields
  ...
  const queryStr = (queryBody) => `query {
    allPeople {
      _id
      name
      ${queryBody}
  ...
  const handleCheck = (pos?: number, all: boolean = false) => {
    const newChecked = checked.map((field, index) => {
      if (all) return (field = true);
  ...
  const generateQueryBody = () => {
    let newQueryBody = "";
    for (const [index, isChecked] of checked.entries()) {
      if (isChecked) newQueryBody += `${fields[index].field}\n`;
    }
  ...
  const getAllButton = () => {
    ...
    const queryString = queryStr(generateQueryBody());
    query(queryString).then((resp) => {
  ...
  const searchOneCharButton = () => {
    ...
    if (addContent.name !== "") {
      const queryString = queryStrName(generateQueryBody());
      query(queryString).then((resp) => {
        ...
        Object.keys(addContentCopy).forEach(
          (string) => (addContentCopy[string] = "")
        );
  ...
  const addCharacterButton = () => {
    if (addContent.name !== "") {
      mutate(queryStrAddCharacter).then((resp) => {
        console.log(resp);
      ...
      <div className="mid-container">
        <div className="button-container">
          <div className="get-all-characters">
            <button
              style={{
                backgroundImage: `url(${imageURL["Search All"]})`,
                boxShadow: `0 0 25px ${searchAll}`,
              }}
              onMouseEnter={() => {
          ...
          <div className="search-characters">
            <button
              style={{
                backgroundImage: `url(${imageURL["Search One"]})`,
                boxShadow: `0 0 25px ${searchOne}`,
              }}
              onMouseEnter={() => {
          ...
          <div className="add-character">
            <button
              style={{
                backgroundImage: `url(${imageURL["unknown"]})`,
                boxShadow: `0 0 25px ${add}`,
              }}
              onMouseEnter={() => {
